<template>
    <div>
            ...
            <table id="healthCheck" class="table table-striped">
                ...
                <tr v-for="health of healthData" :key="health.name">
                    ...
                    <td class="text-center">
                        <span :class='getBadgeClass(health.status)' class='badge'>
                            {{ health.status }}
                        </span>
                    </td>
                    <td class="text-center">
        ...
        <b-modal ref="healthModal">
            <h4 v-if='currentHealth' id='showHealthLabel' slot='modal-title' class='modal-title'><span
                class='text-capitalize'>{{ baseName(currentHealth.name) }}</span>
